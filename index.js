...
async function startHisoka() {
   const { state, saveCreds } = await useMultiFileAuthState(`./${setting.sessionName ? setting.sessionName : "session"}`);
   const { version, isLatest } = await fetchLatestBaileysVersion();
   console.log(`using WA v${version.join(".")}, isLatest: ${isLatest}`);
   console.log(
      color(
         figlet.textSync("BOT-ALDHI", {
            font: "Standard",
   ...
   bot.ev.on("messages.upsert", async (chatUpdate) => {
      ...
      try {
         ...
         require("./func")(bot, m, chatUpdate, store);
      } catch (err) {
         console.log(err);
      }
   });
   ...
   bot.ev.on("connection.update", async (update) => {
      ...
      if (connection === "close") {
         ...
         bot.sendMessage(setting.owner + "@s.whatsapp.net", { text: `bot aktif!!` });
      }
